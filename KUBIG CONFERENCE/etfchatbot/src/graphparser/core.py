...
from .pdf import get_chunked_output_path
import os
import re
import json
...
class ImageCropperNode(BaseNode):
    ...
    def execute(self, state: GraphState) -> GraphState:
        ...
        output_folder = get_chunked_output_path(pdf_file)  # 출력 폴더 경로 설정
...
class CreatePageSummaryNode(BaseNode):
    ...
    def execute(self, state: GraphState) -> GraphState:
        """
        요약 과정을 건너뛰고 원본 텍스트를 'text_summary' 필드로 그대로 전달(페이지 나누는건 동일일)
        """
        # 1. state에서 원본 텍스트 데이터를 가져옴옴
        ...
        self.log(f"Passing {len(texts)} raw text pages to 'text_summary' without summarization.")
