    ...
    with torch.no_grad():
        ...
        for dataset in datasets:
            ...
            for i in range(len(provider)):
                ...
                if lb.dim() == 3 and lb.shape[0] == 3:
                    lb = lb.permute(1, 2, 0).contiguous()

                # 여기서 0~1이면 곱하기 255 후 uint8!
                if pred.max() <= 1.1:
                    pred = (pred * 255.0).round().clamp(0, 255).to(torch.uint8)
                else:
                    pred = pred.round().clamp(0, 255).to(torch.uint8)
                if lb.max() <= 1.1:
                    lb = (lb * 255.0).round().clamp(0, 255).to(torch.uint8)
                else:
                    lb = lb.round().clamp(0, 255).to(torch.uint8)
...
def train(opt, logger, rank=0):
    ...
    with trange(opt.startIter + 1, opt.totalIter + 1, dynamic_ncols=True) as pbar:
        for i in pbar:
            model_G.train()
            ####################################
            # 여기서 조건적으로 use_classifier 전환
            if opt.model == "MultiWeightedSRNets_v2" and hasattr(model_G, "use_classifier"):
                if not model_G.use_classifier and i >= 2000:
                    model_G.use_classifier = True
                    logger.info(f"Classifier weights activated at iter {i}")
            ...
            # loss_G.backward()
            # 변경 (가중합 loss)
            loss_l1 = F.l1_loss(pred, lb)
            loss_mse = F.mse_loss(pred, lb)
            loss_G = 0.85 * loss_mse + 0.15 * loss_l1
            print("[DEBUG] Loss (MSE):", loss_mse.item())
            print("[DEBUG] Loss (L1):", loss_l1.item())
            print("[DEBUG] Loss (Total):", loss_G.item())
            loss_G.backward()
            opt_G.step()
