...
class DIV2K(Dataset):
    def __init__(self, scale, path, patch_size, rigid_aug=True, debug=False,gpuNum=1):
        ...
        self.file_list = [str(i).zfill(4)
                          for i in range(1, 801)]  # use both train and valid/ 추가적으로 800개까지이므로 901에서 801로 수정
        self.debug=debug
        self.gpuNum=gpuNum
...
class SRBenchmark(Dataset):
    ...
    def load_data(self):
        # _ims_all = (5 + 14 + 100 + 100 + 105 + 100) * 2 # manga 109의 경우 실제로 105개 밖에 없으므로 수정
        _ims_all = (5 + 14 + 100 + 100 + 100) * 2 # manga 109의 경우 없음
        # for dataset in ['Set5', 'Set14', 'B100', 'Urban100', 'Manga109', 'DIV2K']:
        for dataset in ['Set5', 'Set14', 'B100', 'Urban100', 'DIV2K']:
            if dataset=='DIV2K':
...
class DIV2K_lazy(Dataset):
    def __init__(self, scale, path, patch_size, rigid_aug=True, debug=False, gpuNum=1):
        super(DIV2K_lazy, self).__init__()
        self.scale = scale
        self.sz = patch_size
        self.rigid_aug = rigid_aug
        self.path = path
        ...
        self.length = len(self.file_list) * 100
    ...
    def __getitem__(self, _):
        ...
        dataLR = os.path.join(self.path, "LR", f"X{self.scale}")
        dataHR = os.path.join(self.path, "HR")

        im = np.array(Image.open(os.path.join(dataLR, f"{key}x{self.scale}.png")))
        lb = np.array(Image.open(os.path.join(dataHR, f"{key}.png")))
...
class SRBenchmark_lazy(Dataset):
    def __init__(self, path, dataset_name='Set5', scale=4):
        super(SRBenchmark_lazy, self).__init__()
        self.path = path
        ...
        folder = os.path.join(path, dataset_name, 'HR')
    ...
    def __getitem__(self, index):
        ...
        hr_path = os.path.join(self.path, self.dataset, 'HR', filename)
        ...
        lr_folder = 'LR' if self.dataset == 'DIV2K' else 'LR_bicubic'
        lr_path = os.path.join(self.path, self.dataset, lr_folder, f"X{self.scale}", f"{base}x{self.scale}.png")
